...
DrawioEditor.prototype.updateImage = function ( imageinfo ) {
	...
	const ts = imageinfo.timestamp ? encodeURIComponent( imageinfo.timestamp ) : '';
	this.imageURL = `${ imageinfo.url }${ separator }ts=${ ts }&cb=${ Date.now() }`;
	if ( this.imgType === 'svg' ) {
		this.image.attr( 'data', this.imageURL );
	} else {
		this.image.attr( 'src', this.imageURL );
	}
	const editLink = $( `.drawioeditor-edit[data-target-id="${ this.id }"]` );
	editLink.attr( 'data-img-url', this.imageURL );
	editLink.data( 'imgUrl', this.imageURL );
...
DrawioEditor.prototype.downloadFromWiki = function () {
	...
	const requestURL = this.imageURL + ( this.imageURL.includes( '?' ) ? '&' : '?' ) +
		'cb=' + Date.now();
